...
\begin{document}
\title{Math 160/263 Minitab Assignment \# 6 - Windows Version} 
...
\item  Use the {\bf Calc $>$ Random Data $>$ Discrete} menu command to
simulate 100 random selections of a ball from the urn, and use the 
{\bf Stat $>$ Tables $>$ Tally} menu command to summarize the simulated 
values.

\item  Produce graphical and numerical descriptions of the first 
column of simulated values.  Are the simulated values consistent
with the corresponding probability distribution?

\item  Use the {\bf Calc $>$ Calculator} menu command to find the mean 
and standard deviation of the number on a ball selected at random from the 
urn.

\item  How close is the mean of the simulated values to the
mean of the corresponding probability distribution?  How close is the standard
deviation of the simulated values to the standard deviation
...
use the {\bf Calc $>$ Calculator} menu command to find the average 
of each row of simulated values.
...
\item  Use the {\bf Calc $>$ Calculator} menu command to find the mean 
and standard deviation of the  average number on two balls selected at 
random from the urn with replacement.
...
\item  Use the {\bf Calc $>$ Probability Distributions $>$ Binomial} menu 
command to find the probability that all 20 Americans believe that the economy
is getting better.  (Hint:  Use the Probability Function.)

\item  Use the {\bf Calc $>$ Probability Distributions $>$ Binomial} menu 
command to find the probability that more than two of the 20 Americans 
do not believe that the economy is getting better.  (Hint:  Use the 
Cummulative Probability Function.)

\item  Use the {\bf Calc $>$ Calculator} menu command to find the mean 
and standard deviation of the number of Americans among the 20 that believe 
